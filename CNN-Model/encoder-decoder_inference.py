'''
This script is used to perform inference using a pre-trained CNN_mask_pytorch_model{timestamp}.pth model 
which was trained on LayerSegmentationCNN architecture from CNN_mask_pytorch.py file.
...
# Model architecture (segmentation model from CNN_mask_pytorch.py)
...
def save_sample_predictions(model, images, masks, indices, save_dir, num_samples=8):
    ...
    class_names = ['Background', 'ILM-PR1 Region', 'PR1-BM Region']
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices[:num_samples]):
            ...
            # Get prediction (segmentation output)
            inp = torch.from_numpy(np.transpose(img, (2, 0, 1))).unsqueeze(0).float().to(device)
            pred_logits = model(inp)  # Shape: (1, num_classes, height, width)
...
def main():
    ...
    results_dir = f"inference/results_{timestamp}"
    ...
    with torch.no_grad():
        for batch_imgs, batch_masks, batch_indices in tqdm(dataloader, desc="Processing batches"):
            ...
            # Get predictions for entire batch (segmentation output: class logits)
            outputs = model(batch_imgs)  # Shape: (batch_size, num_classes, height, width)
            
            # Calculate metrics for each sample in batch
            for i in range(len(batch_imgs)):
                sample_pred = outputs[i:i+1]  # Keep batch dimension
                sample_mask = batch_masks[i:i+1]  # Keep batch dimension
                sample_idx = batch_indices[i].item()
                
                overall_metrics, per_class_metrics = calculate_segmentation_metrics(
                    sample_pred, sample_mask
                )
    ...
    print(f"\n📋 PER-CLASS METRICS:")
    class_names = ['Background (Class 0)', 'ILM-PR1 Region (Class 1)', 'PR1-BM Region (Class 2)']
